module MiscFuncs
using DataFrames, DataFramesMeta, FHist
...
function get_theta_esc_slice(_θemiMin::Real, _θemiMax::Real, _tree::DataFrame)
    return θesc = _tree[(_tree.thetaEmitted .> _θemiMin) .& (_tree.thetaEmitted .< _θemiMax),
                        :thetaEscaped]
end
function get_theta_esc_slice(_θemiMin::Real, _θemiMax::Real, _EMin::Real, _EMax::Real,
                             _tree::DataFrame)
    return θesc = _tree[(_tree.thetaEmitted .> _θemiMin) .& (_tree.thetaEmitted .< _θemiMax) .& (_tree.ESum .> _EMin) .& (_tree.ESum .< _EMax),
                        :thetaEscaped] # return slice where all conditions are fulfilled
end
...
function get_reco_efficiency(_θemiMin::Real, _θemiMax::Real, _EMin::Real, _EMax::Real,
    _tree::DataFrame)
    return get_reco_efficiency( _θemiMin, _θemiMax, get_theta_esc_slice( _θemiMin,_θemiMax,_EMin, _EMax, _tree) )
end
